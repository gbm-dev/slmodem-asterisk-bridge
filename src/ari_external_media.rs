...
impl ExternalMediaRequest {
    ...
    pub fn from_env_defaults() -> Self {
        ...
        // Default to ulaw: matches the PJSIP trunk codec, enabling Asterisk's
        // proxy_media bridge mode (true passthrough, no transcoding). With slin,
        // Asterisk must transcode slin↔ulaw on every frame, adding processing
        // artifacts that corrupt modem training sequences.
